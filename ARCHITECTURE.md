...
- 币种
- KDJ/MA/BOLL/BOLL中轨-MA交叉参数、机器人绑定及“币种 × 周期”推送矩阵
- 新闻源
- 翻译配置
- 清理策略
- 巨鲸 provider 配置
...
全局币种管理中的启用状态决定技术策略的前台监控范围，`notify_intervals_by_symbol` 是机器人推送的唯一规则源。键为币种、值为周期数组；显式空对象表示该策略不推送任何组合。旧 `symbols`、`notify_symbols` 和 `intervals` 只用于升级兼容，启动迁移会把旧提醒币种与周期的笛卡尔积转换成等价矩阵。

策略运行器始终为全部启用币种生成固定看板周期告警，只按每个币种自己的矩阵追加抓取额外周期。策略事件进入 `alert_events` 后，通知 worker 在发送前再次读取最新矩阵；已取消的待发组合直接标记为跳过，不调用 webhook。

前台展示周期和后台推送周期是两个概念：

- 前台展示周期：用户当前看哪个周期。
- 后台推送矩阵：每个指标下，哪些币种的哪些周期会触发机器人提醒，支持 `1m / 5m / 15m / 30m / 1h / 4h / 1d`。
